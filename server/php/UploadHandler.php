...
/* modified to add targeted upload folders based on project# and workorder#
 * also to make target folders read/writable and direct thumbnails to local folder
 */
...
class UploadHandler {
    ...
    function __construct($options = null, $initialize = true, $error_messages = null, $project = 'test', $workorder = 'test') {
        $this->options = array(
            'script_url'                       => $this->get_full_url() . '/',
            'upload_dir'                       => '/var/www/upload/Documentation/' . $project . '/' . $workorder . '/',
            'upload_url'                       => 'http://upload.federatedservice.com/Documentation/' . $project . '/' . $workorder . '/',
            'user_dirs'                        => false,
            'mkdir_mode'                       => 0777,
            'param_name'                       => 'files',
    ...
    protected function get_upload_path($_filename = null, $version = null) {
        ...
        if (empty($version)) {
            ...
            if ($version_dir) {
                return $version_dir . $this->get_user_path() . $filename;
            }
            $version_path = $version . '/';
        }
        return $this->options['upload_dir'] . $this->get_user_path() . $version_path . $filename;
    }
    ...
    protected function get_download_url($_filename, $version = null, $direct = false) {
        if (!$direct && $this->options['download_via_php']) {
            $url = $this->options['script_url']
                    . $this->get_query_separator($this->options['script_url'])
                    . 'file=' . rawurlencode($_filename);
            if ($version) {
        ...
        if (empty($version)) {
            ...
            if ($version_url) {
                return $version_url . $this->get_user_path() . rawurlencode($_filename);
            }
            $version_path = rawurlencode($version) . '/';
        }
        return $this->options['upload_url'] . $this->get_user_path() . $version_path . rawurlencode($_filename);
    }
    ...
    protected function get_file_size($_filepath, $clear_stat_cache = false) {
        if ($clear_stat_cache) {
            clearstatcache(true, $_filepath);
        }
        return $this->fix_integer_overflow(filesize($_filepath));
    }

    protected function is_valid_file_object($_filename) {
        $filepath = $this->get_upload_path($_filename);
        if (is_file($filepath) && $_filename[0] !== '.') {
            return true;
        }
        return false;
    }

    protected function get_file_object($_filename) {
        if ($this->is_valid_file_object($_filename)) {
            $file       = new stdClass();
            $file->name = $_filename;
            $file->size = $this->get_file_size(
                    $this->get_upload_path($_filename)
            );
            $file->url  = $this->get_download_url($file->name);
            foreach ($this->options['image_versions'] as $version => $options) {
                if (!empty($version)) {
                    if (is_file($this->get_upload_path($_filename, $version))) {
    ...
    protected function create_scaled_image($_filename, $version, $options) {
        $filepath = $this->get_upload_path($_filename);
        if (!empty($version)) {
        ...
        switch (strtolower(substr(strrchr($_filename, '.'), 1))) {
            case 'jpg':
            case 'jpeg':
        ...
        $success = imagecopyresampled(
                ...
                ) && $write_image($new_img, $newpath, $image_quality);
